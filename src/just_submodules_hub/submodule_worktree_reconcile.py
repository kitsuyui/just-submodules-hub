"""Reconcile managed submodule worktrees to their expected state."""
...
class Result:
    """A single reconcile outcome for one submodule worktree."""
...
def pull_ff_only(
    ...
    """Attempt a fast-forward-only pull and return the reconcile Result."""
...
def switch_default_and_pull(
    ...
    """Fetch default branch, switch to it, and fast-forward pull."""
...
def detached_result(repo: Path, repo_label: str, dirty: str, default: str) -> Result:
    """Handle a detached HEAD: try to settle to the default branch."""
...
def reconcile_one(root: Path, repo_path: str) -> Result:
    ...
    resolved_default = default_branch(repo)
    if not branch:
...
def prefiltered_default_result(root: Path, repo_path: str, default: str) -> Result:
    """Return a noop Result for a prefiltered up-to-date submodule."""
    repo = root / repo_path
    return Result(
        repo_path,
        "noop",
...
def build_reconcile_targets(
    ...
    *,
    ...
    """Split *paths* into targets-to-reconcile and prefiltered Results."""
    if not prefilter:
        return paths, []
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    """Parse command-line arguments for reconcile-submodule-worktrees."""
...
def main(argv: list[str] | None = None) -> int:
    """Reconcile submodule worktrees according to the given mode and print results."""
